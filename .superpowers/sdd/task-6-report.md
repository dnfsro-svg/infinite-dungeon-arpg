...
- RED 仅修改测试后，平台套件为 `380 cases, 2 failures`：完整深渊效果缺失，且 1024×704 的目标面板不足以容纳四行；失败原因与 P1 完全一致。
- 深渊目标面板现使用四个固定容量字段：战斗目标、危险度与完整规则、完整效果、带 `待领奖励`/`未领取` 标签的计数。面板使用 640×120 逻辑尺寸，1024×704 下位于中央且不侵占玩家、导航或战斗排除区域。
- P2 缓存测试拆为两个独立用例：仅改变 `density_affix` 和仅改变 `initial_monster_count`；每项分别断言 rebuild 增量和对应文本变化。
- GREEN：平台 `380/380`；HUD 100k 零分配、HUD 边界及 mutation self-test、HUD 证据 guard 及 self-test、platform module boundary 共 `7/7`；dungeon `296/296`、persistence `94/94` 均通过；`git diff --check` 通过（仅换行转换警告）。
- 修复未修改 density 会话、保存、战斗、相机、37 条房间网格或 Stage 11-C 三项既有语义。
