...
单线程构建通过后，保持原输入和 `24000 + 96N` 运行一次：exit 8，81,600
tick，599/600。唯一未击败目标为 ordinal 201，home cell 132（row 6、
column 12）；它曾 resident，但最后一次可见与 home-cell 覆盖均停在 tick 7,640。
全程名义累计覆盖 400/400 格，却记录到 39 次非零 sweep 输入后玩家原地不动，
且 10 个 waypoint 均有阻挡。由此排除“cell 永不生成/永不驻留”，确认实际路线
在障碍处跳段后没有重新访问仍存活目标所在区域。
